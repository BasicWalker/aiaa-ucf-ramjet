...
% flight properties
initial_mach = 2.5;  % booster max mach
initial_altitude = 1000;  %  initial altitude for ramjet start <m>
design_mach = 3.5;  % mach number for criticl flight operations
burntime = 7;  % burntime to reach design mach <sec>
...
while true
    ...
    density(j,1) = interp1(GRAM.Hgtkm, GRAM.DensMean, (altitude(j,1))/1e3);
    pressure(j,1) = interp1(GRAM.Hgtkm, GRAM.PresMean, (altitude(j,1))/1e3);
    temperature(j,1) = interp1(GRAM.Hgtkm, GRAM.Tmean, (altitude(j,1))/1e3);
    ...
    acceleration(j,1) = (thrust(j,1) - drag(j,1))/ mass(j,1);  % - weight(j,1)
    ...
    for i = 2:size(t,2)
        ...
        density(j,i) = interp1(GRAM.Hgtkm, GRAM.DensMean, (altitude(j,i))/1e3);
        pressure(j,i) = interp1(GRAM.Hgtkm, GRAM.PresMean, (altitude(j,i))/1e3);
        temperature(j,i) = interp1(GRAM.Hgtkm, GRAM.Tmean, (altitude(j,i))/1e3);
        ...
        acceleration(j,i) = (thrust(j,i) - drag(j,i))/ mass(j,i);  % - weight(j,i)
    end
...
ax1 = subplot(4,1,1);
plot(ax1,t,altitude(end,:));
title(ax1,'Altitude');
ylabel(ax1,'<m>');
xlabel(ax1,'<s>');
ax2 = subplot(4,1,2);
plot(ax2,t,temperature(end,:));
title(ax2,'Temperature');
ylabel(ax2,'<K>');
ax3 = subplot(4,1,3);
plot(ax3,t,pressure(end,:));
title(ax3,'Pressure');
ylabel(ax3,'<Pa>');
ax4 = subplot(4,1,4);
plot(ax4,t,density(end,:));
title(ax4,'Density');
ylabel(ax4,'<kg/m^3>');
