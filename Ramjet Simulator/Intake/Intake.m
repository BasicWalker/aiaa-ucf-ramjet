...
[intake.mach(2,n), intake.shockAngle(n)] = obliqueShock(intake.mach(1,n), intake.DeflAngle, gamma);  % uniform mach number of external ramp region
...
[~, ~, ~, ~, ~, intake.r_n_chokeLimit(n)] = flownormalshock(gamma, intake.mach(2,n), 'mach');  % ratios are downstream over upstream
if (intake.stagPres(2,n)*intake.r_n_chokeLimit(n)) < intake.chokeStagPres(n)
...
[intake.mach(3,n), intake.tempRatio(4,n), intake.presRatio(4,n), intake.densRatio(4,n), intake.mach(4,n), intake.stagPresRatio(4,n), ~]...
    = flownormalshock(gamma, intake.r_n, 'totalp');
...
[~, intake.tempRatio(3,n), intake.presRatio(3,n), intake.densRatio(3,n), intake.A_Astar(3,n)]...
    = flowisentropic(gamma, intake.mach(3,n), 'mach'); % ratios are static over stagnation
...
[~, ~, ~, ~, intake.A_Astar(4,n)] = flowisentropic(gamma, intake.mach(4,n), 'mach');
...
if intake.A_Astar(5,n) < 1
    error('intake:CombustorInletChoke',...
        'Error. \nCombustor inlet too small; flow reaches Mach 1. Flow is choked\n');
end


[intake.mach(5,n), intake.tempRatio(5,n), intake.presRatio(5,n), intake.densRatio(5,n), ~] = ...
    flowisentropic(gamma, intake.A_Astar(5,n), 'sub');  % ratios are static over stagnation
...
intake.velocity(5,n) = intake.mach(5,n)*sqrt(gamma*R*intake.staticTemp(5,n));  % <m/s>
