...
while resid > eps('single')
    ...
    drag(1) = c_d*S*0.5*density(1)*velocity(1)^2;
    mass(1) = wet_mass;
    weight(1) = g*mass(1);
    acceleration(1) = (thrust(1) + drag(1) + weight(1))/ mass(1);

    for i = 2:size(t,2)
        acceleration(i) = (thrust(i) + drag(i) + weight(i))/ mass(i);
        
        
        velocity(i) = velocity(i-1) + acceleration(i)*(step_size);
