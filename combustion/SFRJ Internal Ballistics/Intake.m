...
% Station 1
[~, Ti_To1(n), Pi_Po1(n), Rho_Rho_o(n), ~] = flowisentropic(gamma, flight_mach(n));         % Isentropic flow properties at flight mach
T_o1(n) = (1/Ti_To1(n)) * Temp_atm(n);                                                      % Stag Temp 1
Po1(n) = (pressure_atm(n)/Pi_Po1(n));                                                       % Stag Pres 1
Rho_o(n) = Rho_atm(n)/Rho_Rho_o(n);                                                         % Stag Density 1
v_1(n) = flight_mach(n)*sqrt(gamma*R*Temp_atm(n));                                          % Velocity at station 1
         
% Station 2 (oblique shock) 
[mach_2(n), beta_angle(n)] = obliqueShock(flight_mach(n), def, gamma);                      % Oblique shock properties at flight mach
mach_2_normal(n) = flight_mach(n)*sind(beta_angle(n));                                      % Normal component of mach after oblique shock
...
T_2(n) = T2_T1(n)*Temp_atm(n);                                                              % Static Temp 2
